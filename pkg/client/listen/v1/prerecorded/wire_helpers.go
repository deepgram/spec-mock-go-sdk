...
func requireWired(t *testing.T, input any, field string) {
	...
	if !f.IsValid() {
		t.Fatalf("spectypes.TranscribeInput has no field %q yet — spec needs to model it before the converter can wire it through.", field)
	}
	if isZeroForWire(f) {
		t.Fatalf("spectypes.TranscribeInput.%s exists but optionsToTranscribeInput didn't wire it.", field)
	}
}
...
func requireDropped(t *testing.T, input any, field string) {
	...
	if !isZeroForWire(f) {
		t.Fatalf("spectypes.TranscribeInput.%s should not be wired.", field)
	}
}
