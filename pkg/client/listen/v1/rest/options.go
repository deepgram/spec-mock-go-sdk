...
type PreRecordedTranscriptionOptions struct {
	...
	// AdditionalQueryParams is an escape hatch for passing arbitrary query
	// parameters that are not (yet) modelled on the public options surface.
	// Values here are appended to the request query string as-is and have
	// no counterpart on the generated spectypes.TranscribeInput wire shape.
	// Use sparingly; prefer adding a typed field when a parameter graduates
